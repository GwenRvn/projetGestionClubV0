    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        ...
        <form action="{{ route('users.update', $user) }}" method="POST"
            ...
            <div class="px-4 py-5 bg-white sm:p-6">
                ...
                <div class="py-2">
                    <label for="licence" class="block text-sm font-medium text-gray-700">Licence</label>
                    <input type="text" name="licence" id="licence"
                           autocomplete="given-name"
                           class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"
                           value="{{ old('licence', $user->licence) }}">
                    @error('licence')
                    <span class="text-red-600">{{ $message }}</span>
                    @enderror
                </div>
...
items-center w-1/4 py-4 border border-gray-400 shadow-sm text-base font-medium rounded-md text-gray-700 bg-white justify-center"
                           value="Modifier">
                </div>
            </div>
        </form>
